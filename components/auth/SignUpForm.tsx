    ...
    const handleVerificationSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (result.status === "complete") {
                ...
                setVerificationError("😖 Sorry could not verify you. 😊 Please try again.")
            }
        } catch (error: any) {
            console.error("😖 Verification error :", error);
            setAuthError(error.errors?.[0]?.message || "😖 An error occurred during verification. 😊 Please try again.");
        }
        finally {
            setIsSubmitting(false);
        }
        ...
        <>
            <Card className="w-full max-w-md border border-default-200 bg-default-50 shadow-xl">
                <CardHeader className="flex flex-col gap-1 items-center pb-2">
                    ...
                    <p className="text-default-500 text-center">
                        Sign up to start managing your digital assests securely
                    </p>
                </CardHeader>
                <Divider />
                <CardBody className="py-6">
